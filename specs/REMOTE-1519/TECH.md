...
- Click the chip on a non-eligible conversation (no synced server token); verify the pane opens as a fresh cloud-mode pane with no handoff context (existing fall-through preserved).
